...
## Confirmed stats (re-derived directly, 2026-07-23 — supersedes rounded figures in the report where they differ)

On `04_instrumentsdf_bondgraph2.pkl` (the final feature set, 59,820 rows):
- Date range: 2013-07-23 to 2018-12-18.
- 132 unique sellers (`customer_id`), 3,176 unique buyers (`debtor_id`) — close to but not identical to
  the report's rounded "115 sellers, 3,219 buyers" (likely a slightly different filtering stage).
...
- Graph construction (nodes = buyers/sellers/hybrids, edges = trade relationships) can be derived directly
  from `00_transactionsdf_simNames.pkl` or `01_instrumentsdf.pkl` using the same logic described in
  `wiki/original-project/data-and-network-construction.md`, rather than needing to reverse-engineer it from
  scratch.
